...
// register demo API 
add_action('rest_api_init', 'reg_conn_router');

function reg_conn_router() {
	register_rest_route(
		'conn_test/v1',
		'/demo',
		array(
 ...
 */
function conn_handle_transaction( $response, $handler, $request ) {
	global $conn;
	$total = null;
...
add_filter('rest_request_after_callbacks', 'conn_handle_transaction', 9, 3);
 ...
 */
function conn_start_transaction( $response, $handler, $request ) {
	global $conn;
...
add_filter('rest_request_before_callbacks', 'conn_start_transaction', 9, 3);
